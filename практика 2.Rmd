...
library('data.table')
```
Условие задания: Построить график динамики суммарной массы поставок в январе-августе (пропуски,
заменённые на средние, Netweight.kg.mean) по годам. Цветом показать три группы
стран-поставщиков: страны Таможенного союза, остальные страны СНГ и все
остальные страны.
...
```
Была проведена на наличие пропусков и замена их на среднее значение.
```{r}
...
```
Далее  был сформирован массив по странам согласно условию задания на три таблицы и соеденены вооедино.
...
```
Посчитали суммарные значения по странам и годам.
...
и Приступили к построению графиков с помощью необходимых пакетов.
```{r}
